...
// Count
$countStmt = $db->prepare("SELECT COUNT(*) FROM barangays b $whereSQL");
...
// Fetch
$stmt = $db->prepare("SELECT b.* FROM barangays b $whereSQL ORDER BY b.name ASC LIMIT $perPage OFFSET $offset");
$stmt->execute($params);
$rows = $stmt->fetchAll();
?>
...
<div class="page-header">
    <div>
        <div class="page-header-title"><i class="bi bi-geo-alt-fill me-2 text-primary"></i>Barangays</div>
        <div class="page-header-sub">Manage barangay master list and information</div>
    </div>
    ...
    <div class="toolbar">
        <form method="GET" class="d-flex gap-2 flex-grow-1">
            <input type="text" name="search" class="form-control" style="max-width:240px;"
                   placeholder="Search name / description…" value="<?= clean($search) ?>">
...
<div class="card" style="border-radius:0 0 var(--radius) var(--radius);">
    <div class="table-wrapper">
        <table class="table mb-0">
            ...
            <tbody>
            <?php if (empty($rows)): ?>
                <tr><td colspan="5" class="empty-state"><i class="bi bi-inbox"></i><p>No barangays found.</p></td></tr>
        ...
        <div class="modal-content">
            <form id="recordForm" method="POST" action="save.php">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle">Add Barangay</h5>
